...
module.exports = createCoreController("api::contact-form.contact-form", () => ({
  async findFolder(folderName = "", parentId = null) {
    try {
      const existingFolders = await strapi.entityService.findMany(
  ...
  async create(ctx) {
    try {
      ...
      const attachment = files?.["files.attachment"] || files?.attachment;
      ...
      if (attachment) {
        ...
        try {
          ...
          strapi.log.info(
            `File uploaded successfully: ${uploadedFile?.name} (ID: ${uploadedFile?.id})`
          );
        } catch (uploadError) {
      ...
      const entryData = {
        ...parsedData,
        attachment: uploadedFile?.id || null,
      };
      ...
      strapi.log.info(`Contact form created successfully: ${entry?.id}`);
